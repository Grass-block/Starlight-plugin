...
public interface BukkitUtil {
    ...
    MethodHandleRS0<double[]> TPS = MethodHandle.select((ctx) -> {
        ...
        ctx.dummy(() -> new double[]{PlatformPatcher.instance().map((c) -> c.tps().get()).orElse(20.0)});
    });
    MethodHandleRS0<Double> MSPT = MethodHandle.select((ctx) -> {
        ...
        ctx.dummy(() -> SpigotReflection.instance().mspt());
    });
    ...
    @SafeVarargs
    static <E extends Event> Future<E> callEvent(E event, Consumer<E>... outcomes) {
        var f = new CompletableFuture<E>();
    ...
    @SafeVarargs
    static <E extends Event> E callEventUnsafe(E event, Consumer<E>... outcomes) {
        try {
            Bukkit.getPluginManager().callEvent(event);
    ...
    static String formatTPS(double tps) {
        var col = (tps > 17 ? ChatColor.GREEN : tps > 10 ? ChatColor.YELLOW : ChatColor.RED);
    ...
    static void createDrop(Location loc, ItemStack item) {
        ...
        Item e;
        try {
            e = loc.getWorld().spawn(loc, Item.class);
            e.setItemStack(Objects.requireNonNull(item));
        } catch (IllegalArgumentException e3) {
            e = loc.getWorld().dropItem(loc, item);
        }

        BukkitUtil.callEvent(new ItemCreateEvent(e));
    }
