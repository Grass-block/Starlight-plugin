...
public final class SimpleRecordEntry implements RecordEntry {
    ...
    public void open() {
        File f = new File(FILE_LOCATION.formatted(FilePath.pluginFolder("starlight"), this.id));

        if (!f.exists() || f.length() == 0) {
            if (f.getParentFile().mkdirs()) {
                Starlight.instance().getLogger().info("created record folder of " + f.getName());
            }
            try {
                if (f.createNewFile()) {
                    Starlight.instance().getLogger().info("created record file " + f.getName());
                }
            } catch (Exception e) {
    ...
    public void close() {
        try {
            ...
            Starlight.instance().getLogger().info("failed to close record entry %s: %s".formatted(this.id, e.getMessage()));
        }
    }
}
