package org.atcgroup.starlight.bundle.management;
...
public final class PluginManagerCommand extends SLCommandModule {
    ...
    private void enablePlugin(CommandSender sender, String name) {
        ...
        if (p == null) {
            MessageAccessor.send(this.language, sender, "plugin-not-found", name);
            return;
        }

        this.bukkitPluginManager.enablePlugin(p);
        MessageAccessor.send(this.language, sender, "enable", name);
    }
    ...
    private void disablePlugin(CommandSender sender, String name) {
        ...
        MessageAccessor.send(this.language, sender, "disable", name);
    }
    ...
    private void restartPlugin(CommandSender sender, String name) {
        ...
        this.bukkitPluginManager.disablePlugin(p);
        this.bukkitPluginManager.enablePlugin(p);
        MessageAccessor.send(this.language, sender, "restart", name);
    }
    ...
    private void loadPlugin(CommandSender sender, String fileName) {
        ...
        if (!file.exists()) {
            MessageAccessor.send(this.language, sender, "file-not-found", fileName);
            return;
        }

        if (this.pluginManager.load(fileName) != null) {
            MessageAccessor.send(this.language, sender, "load", fileName);
        } else {
            MessageAccessor.send(this.language, sender, "load-failed", fileName);
        }
    }
    ...
    private boolean unloadPlugin(CommandSender sender, String name) {
        ...
        if (this.pluginManager.unload(p)) {
            MessageAccessor.send(this.language, sender, "unload", name);
            return true;
        } else {
            MessageAccessor.send(this.language, sender, "unload-failed", name);
            return false;
        }
    }
    ...
    private void listPlugins(CommandSender sender) {
        ...
        MessageAccessor.send(this.language, sender, "list", "");
    ...
    private void sendPluginInfo(CommandSender sender, String name) {
        ...
        builder.append("  ")
                .append(MessageAccessor.getMessage(
                        language,
                        locale,
                        "info-name",
                        desc.getName(),
                ...
                .append("\n");
        builder.append("  ").append(MessageAccessor.getMessage(language, locale, "info-version", desc.getVersion())).append("\n");
        builder.append("  ")
                .append(MessageAccessor.getMessage(language, locale, "info-author", Arrays.toString(desc.getAuthors().toArray())))
                .append("\n");
        builder.append("  ")
                .append(MessageAccessor.getMessage(language, locale, "info-depend", Arrays.toString(desc.getDepend().toArray())))
                .append("\n");
        builder.append("  ")
                .append(MessageAccessor.getMessage(language, locale, "info-soft-depend", Arrays.toString(desc.getSoftDepend().toArray())))
                .append("\n");
        builder.append("  ").append(MessageAccessor.getMessage(language, locale, "info-api", desc.getAPIVersion())).append("\n");
        builder.append("  ").append(MessageAccessor.getMessage(language, locale, "info-main", desc.getMain())).append("\n");
        builder.append("  ")
                .append(MessageAccessor.getMessage(
                        language,
                        locale,
                        "info-desc",
                        desc.getDescription() == null ? "[empty]" : desc.getDescription()
                ));

        /*
        //builder.append("  ").append(MessageAccessor.getMessage(language,locale, "info-libs")).append("\n");
        for (String s : desc.getLibraries()) {
            builder.append(ChatColor.GOLD).append("     - ").append(ChatColor.WHITE).append(s).append("\n");
        }
         */

        MessageAccessor.send(this.language, sender, "info", name, builder.toString());
    }
}