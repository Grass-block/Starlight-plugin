...
public final class TabSync extends VelocityAbstractModule {
    ...
    public void render(RegisteredServer server) {
        var players = this.server.getAllServers()
                .stream()
        ...
        for (var viewer : server.getPlayersConnected()) {
            ...
            players.forEach((p) -> p.getCurrentServer().ifPresent((s) -> this.render(tab, p, s.getServerInfo())));
        }
    }
    ...
    public void renderPlayerTo(Player player, ServerInfo server) {
        for (var viewer : getProxyServer().getAllPlayers()) {
    ...
    public void removePlayer(Player player) {
        getProxyServer().getAllPlayers().stream().filter((p) -> p != player).forEach((p) -> {
