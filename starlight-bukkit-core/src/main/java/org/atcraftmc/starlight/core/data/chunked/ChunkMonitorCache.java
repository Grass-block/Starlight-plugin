...
import org.atcraftmc.starlight.core.data.region.WorldAABB;
...
public final class ChunkMonitorCache<V extends UUIDMapped> {
    ...
    public ChunkMonitorCache(String world, ChunkedDataProvider<V> listener) {
        this.world = world;
    ...
    private VirtualChunk loadVirtualChunk(int cx, int cz) {
        ...
        for (var r : data) {
            try {
                var container = this.getRegionCache().get(r.getUuid(), () -> new ChunkedObjectContainer<>(r)).set(r);
    ...
    public void handleRegionCacheRemove(RemovalNotification<UUID, ChunkedObjectContainer<V>> notification) {
    ...
    public ChunkedDataProvider<V> getListener() {
        return listener;
    }

    public Cache<UUID, ChunkedObjectContainer<V>> getRegionCache() {
        return regionCache;
    }
