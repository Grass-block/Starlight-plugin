...
TPSBar 模块会在玩家屏幕顶部的BossBar上持续显示当前服务器的TPS（Tick Per Second）和MSPT（Milliseconds Per
Tick）信息。根据MSPT数值高低，BossBar颜色会在绿色（<15ms）、黄色（<35ms）和红色（>=35ms）之间切换，进度条反映MSPT占50ms的比例。玩家可通过指令切换显示或隐藏。
