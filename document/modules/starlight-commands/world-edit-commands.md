...
为WorldEdit玩家补充一系列便捷指令：选区边框绘制(`/outline-box`)、选区镜像(`/mirror`)、快速排水(`/drain-water`)
、快速树种画笔(`/fast-brash`)。所有指令均基于WorldEdit API实现，需要服务端安装WorldEdit插件。其中`/fast-brash`
预设了30余种树木和草方块画笔模板，方便快速植林。
