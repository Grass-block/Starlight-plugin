...
public final class DynamicViewDistance extends SLPackageModule implements PluginCommandExecutor {
    ...
    public void checkCompatibility() {
        Compatibility.blackListPlatform(APIProfile.ARCLIGHT);
    ...
    public void calculatePlayerViewDistance(final Player player, boolean remind, boolean forceRemind) {
        ...
        if ((programRemind && remind) || forceRemind) {
            MessageAccessor.send(this.language, player, "set-self", value);
        }
    ...
    public void execute(CommandExecution context) {
        ...
        if (value == -1) {
            ...
            if (!isSelf) {
                MessageAccessor.send(this.language, sender, "reset-target", args[0]);
            }
            MessageAccessor.send(this.language, player, "reset-self");
        ...
        if (!isSelf) {
            MessageAccessor.send(this.language, sender, "set-target", args[1], value);
        }
        MessageAccessor.send(this.language, player, "set-self", value);
